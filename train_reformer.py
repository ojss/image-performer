...
def parse_args_and_config():
    ...
    parser.add_argument(
        '--config', type=str, default='transformer_dmol.yml', help='Path to the config file')
            ...
            if step % config.train.sample_iter == 0:
                ...
                if config.model.distr == "cat":
                    ...
                    for idx, c in enumerate(channels):
                        ax = sns.heatmap(losses_per_dim[idx, :, :].cpu(
                        ).numpy(), linewidth=0.5, cmap=color_codes[idx])
                        tb_logger.add_figure(
                            "losses_per_dim/{}".format(c), ax.get_figure(), close=True, global_step=step)
                else:
                    ax = sns.heatmap(losses_per_dim[0, :, :].cpu(
                    ).numpy(), linewidth=0.5, cmap='Blues')
                    tb_logger.add_figure(
                ...
                with torch.no_grad():
                    ...
                    if args.sample:
                        samples = revert_samples(model.sample(
                            config.train.sample_size, config.device))
                        samples_grid = torchvision.utils.make_grid(
                            samples[:8, ...], 3)
                        tb_logger.add_image(
                            'samples', samples_grid, global_step=step)
