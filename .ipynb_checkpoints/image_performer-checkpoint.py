...
class ImagePerformer(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        if self.hparams.distr == "dmol":  # Discretized mixture of logistic, for ordinal valued inputs
            ...
            self.output_dense = nn.Linear(
                self.hparams.hidden_size, depth, bias=False)
    ...
    def forward(self, X, sampling=False):
        ...
        if sampling:
            ...
            X = X.view(X.shape[0], X.shape[1], X.shape[2] *
                       X.shape[3])  # Flatten channels into width
        ...
        if self.hparams.distr == "dmol":
            ...
            channel_addition = (torch.tensor(
                [0, 1, 2]) * NUM_PIXELS).to(X.device).repeat(X.shape[2] // 3).view(1, 1, -1)
            X += channel_addition
        ...
        if not sampling and self.hparams.distr == "cat":  # Unpack the channels
            X = X.view(X.shape[0], X.shape[1], X.shape[2] //
                       self.hparams.channels, self.hparams.channels, X.shape[3])
    ...
    def split_to_dml_params(self, preds, targets=None, sampling=False):
        ...
        if not sampling:
            ...
            locs2 = locs[..., 2, :] + coeffs[..., 1, :] * targets[:,
                                                                  0, ...] + coeffs[..., 2, :] * targets[:, 1, ...]
    ...
    def loss(self, preds, targets):
        if self.hparams.distr == "dmol":
            ...
            ce = F.cross_entropy(preds.permute(
                0, 4, 1, 2, 3), targets, reduction='none')
            return ce
    ...
    def accuracy(self, preds, targets):
        if self.hparams.distr == "cat":
            ...
            acc = torch.eq(argmax_preds, targets).float().sum() / \
                np.prod(argmax_preds.shape)
            return acc
        else:
    ...
    def sample_from_dmol(self, outputs):
        ...
        gumbel_noise = - \
            torch.log(-torch.log(torch.rand_like(logits)
                      * (1. - 2 * 1e-5) + 1e-5))
    ...
    def sample_from_cat(self, logits, argmax=False):
        if argmax:
            ...
            gumbel_noise = - \
                torch.log(-torch.log(torch.rand_like(logits)
                          * (1. - 2 * 1e-5) + 1e-5))
            sel = torch.argmax(logits + gumbel_noise, -1,
                               keepdim=False).float() / 255.
        return sel
    ...
    def sample(self, n, device, argmax=False):
        ...
        for curr_infer_length in tqdm(range(total_len)):
            ...
            outputs = outputs.view(
                n, -1, outputs.shape[-1])[:, curr_infer_length:curr_infer_length+1, :]
            if self.hparams.distr == "dmol":
        ...
        samples = samples.view(n, self.hparams.image_size,
                               self.hparams.image_size, self.hparams.channels)
...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        # self.attn.to_v = nn.Linear(self.hparams.hidden_size, self.vd, bias=False)
        # self.attn.to_out = nn.Linear(self.vd, self.hparams.hidden_size, bias=False)
...
class Attn(nn.Module):
    def __init__(self, hparams):
        super().__init__()
        # self.attn_fn = FastAttention(dim_heads=64, nb_features=256)
        self.hparams = hparams
        self.kd = self.hparams.total_key_depth or self.hparams.hidden_size
        self.vd = self.hparams.total_value_depth or self.hparams.hidden_size
    ...
    def forward(self, X):
        ...
        # result = self.attn_fn(q, k, v)

        # Split to shape [batch_size, num_heads, len, depth / num_heads]
        q = q.view(q.shape[:-1] + (self.hparams.num_heads,
                   self.kd // self.hparams.num_heads)).permute([0, 2, 1, 3])
        k = k.view(k.shape[:-1] + (self.hparams.num_heads,
                   self.kd // self.hparams.num_heads)).permute([0, 2, 1, 3])
        v = v.view(v.shape[:-1] + (self.hparams.num_heads,
                   self.vd // self.hparams.num_heads)).permute([0, 2, 1, 3])
        q *= (self.kd // self.hparams.num_heads) ** (-0.5)

        if self.hparams.attn_type == "global":
            bias = -1e9 * \
                torch.triu(torch.ones(X.shape[1], X.shape[1]), 1).to(X.device)
            ...
            if q.shape[2] > blen:
                ...
                tail_q = q[:, :, 1:]
                bias = -1e9 * \
                    torch.triu(torch.ones(blen, 2 * blen),
                               blen + 1).to(X.device)
